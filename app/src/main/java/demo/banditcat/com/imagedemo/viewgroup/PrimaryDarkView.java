...
public class PrimaryDarkView extends RelativeLayout {
    ...
    public void binder(AliPaymentModel aliPaymentModel) {
        ...
        if (!aliPaymentModel.getDateTimeStyle()) {
            ...
            mCalendar.setTimeInMillis(TimeUtils.millis2millis(aliPaymentModel.getTopTime(),TimeUtils.DEFAULT_PATTERN_6));
            int apm = mCalendar.get(Calendar.AM_PM);

            if (apm == 1) {
                topDateTime.setText("下午 " + TimeUtils.millis2String(mCalendar.getTimeInMillis(), TimeUtils.DEFAULT_PATTERN_4_1));

            } else {
                topDateTime.setText("上午 " + TimeUtils.millis2String(mCalendar.getTimeInMillis(), TimeUtils.DEFAULT_PATTERN_4_1));
            }
        } else {
